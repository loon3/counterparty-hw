...
import { encodeMessageRC4, bin2hex } from './util.js'
...
export function createIssuanceMessageOpreturn(asset, quantity, divisible, lock, reset, description, callback) {
    ...
    if(divisible){
        quantityInt = new Decimal(quantity).toDecimalPlaces(8).times(1e8).toNumber();
        divisibleHex = "01"
    }
    if(lock){
        lockHex = "01"
    ...
    const issuanceParams = divisibleHex + lockHex +resetHex
        
    if (quantityInt <= 18446744073709551615) {
        ...
        var quantity_hex = padprefix(quantityInt.toString(16),16);

        var description_hex = bin2hex(description);

        var issuance_tx_data = cntrprty_prefix + trans_id + assetid_hex + quantity_hex + issuanceParams + description_hex;

        console.log(issuance_tx_data.length);

        if(issuance_tx_data.length <= 160){
            callback(issuance_tx_data);
        } else {
...
export function createTxIssuanceOpreturn(issuanceData, callback){
            
    getutxos(issuanceData.fromAddress, issuanceData.txFeeSatoshis, function(total_utxo, satoshi_change){ 

        createIssuanceMessageOpreturn(issuanceData.asset, issuanceData.quantity, issuanceData.divisible, issuanceData.lock, issuanceData.reset, issuanceData.description, function(datachunk_unencoded){
...
//
//export function csvCheckSigOutput(aQ, bQ, lockTime) {
//  const alicePrivkey = getPrivkeyFromPassphrase("alas rabbit guard familiar forever crimson knock thorn spell jeans silence nerd", {"address":"1cFvR36qwkwcXxEbjtgDtPbYTvgrzGoue","derivationPath":"m/0'/0/0","format":"legacy","formatType":"LEGACY","index":0,"key":"passphrase"})
//  const bobPrivkey = getPrivkeyFromPassphrase("alas rabbit guard familiar forever crimson knock thorn spell jeans silence nerd", {"address":"1JU92np2EFwRXJf14WWFmPcHS4URUviUm6","derivationPath":"m/0'/0/1","format":"legacy","formatType":"LEGACY","index":1,"key":"passphrase"})
//    
//  const keyPairAlice1 = bitcoinjs.ECPair.fromWIF(alicePrivkey)
//  const keyPairBob1 = bitcoinjs.ECPair.fromWIF(bobPrivkey)  
//    
...
//          OP_DROP
//      OP_ELSE
//          ${keyPairBob1.publicKey.toString('hex')}
//          OP_CHECKSIGVERIFY
//      OP_ENDIF
//      ${keyPairAlice1.publicKey.toString('hex')}
//      OP_CHECKSIG
//    `
//      .trim()
//      .replace(/\s+/g, ' '),
