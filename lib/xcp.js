...
function createSendMessageOpreturn(address, assetName, amount, divisible, callback) {
    ...
    if(divisible){
        var amount_round = parseInt(amount*100000000)
    } else {
...
export function createTxSendAssetOpreturn(add_from, add_to, asset, asset_total, divisible, transfee, callback){
        
    getutxos(add_from, transfee, function(total_utxo, satoshi_change){ 

        createSendMessageOpreturn(add_to, asset, asset_total, divisible, function(datachunk_unencoded){
...
export function recommendedFee(callback){
    ...
    const default_txsize_byte = 264 //one input, two outputs    
    ...
    getRecommendedFeeRate(function(data){
        ...
        callback(fee_recommended_economy)
